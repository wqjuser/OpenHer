...
make doctor         # 本地配置/数据体检，不会调用外部 provider
make doctor-strict  # 本地严格体检；warning 也返回非零
...
make backend-chat-smoke        # 启动真实 uvicorn，走一轮 /api/chat、session status 和历史记录
